...
USAGE: 'python process_NCEI_02_serial.py NLCD_2011_WLS_UTM15N NCEI_WLS_19840101-20131231 ./grids 500 RBF 1'
...
NOTES: RBF is an exact full-domain method with an excellent daily processing time. CSP is just 
       about as fast but may not cover the whole desired domain, forming a convex hull around 
       the available stations for each grid, can get weird in areas with low station density, 
       and produces some weird patterns between stations. BSP does cover the whole desired 
       domain but also gets weird in areas with low station density, especially near the edges 
       of the domain. IDW maps often show isolated bulls-eye 
...
RUN TIME: ~60 sec per data day (both IDW and RBF seem to take the same time)
"""
...
def process_grid_idw(grid_x,grid_y,grid_shape,stns_e,stns_n,stns_vals,exp):
    grid_var_flat = Interpolation.simple_idw(stns_e, stns_n, stns_vals, grid_x, grid_y, exp)
    grid_var = grid_var_flat.reshape(grid_shape)
    return grid_var

def process_grid_csp(grid_x,grid_y,stns_e,stns_n,stns_vals):
    grid_var = Interpolation.scipy_griddata(stns_e, stns_n, stns_vals, grid_x, grid_y)
    grid_var = grid_var.T
    return grid_var

def process_grid_bsp(grid_x,grid_y,stns_e,stns_n,stns_vals):
    grid_var = Interpolation.scipy_bspline(stns_e, stns_n, stns_vals, grid_x, grid_y)
    return grid_var

def process_grid_rbf(grid_x,grid_y,stns_e,stns_n,stns_vals):
    grid_var = Interpolation.scipy_rbf(stns_e, stns_n, stns_vals, grid_x, grid_y)
    return grid_var

message(' ')
message('process_NCEI_02_serial.py started at %s' % datetime.datetime.now().isoformat())
message(' ')
#
if len(sys.argv) < 7:
...
if interp_method == 'RBF':
    ...
    message('generating target grid for multiquadric radial basis function (RBF) interpolation')
    ...
    # interpolate using the whole target grid
    message('generating target grid for inverse-distance-weighted (IDW) interpolation')
    grid_x = np.arange(min_x, max_x, dx)
    grid_y = np.arange(min_y, max_y, dy)
    grid_x, grid_y = np.meshgrid(grid_x, grid_y)
    grid_shape = grid_x.shape
...
for date in dates:
    ...
    if interp_method == 'RBF':
        grid_prcp = process_grid_rbf(grid_x,grid_y,prcp_east,prcp_north,prcp_vals)
        ...
        grid_tmax = process_grid_rbf(grid_x,grid_y,tmax_east,tmax_north,tmax_vals)
        message('- calculated TMAX grid via %s' % interp_method)
        grid_tmin = process_grid_rbf(grid_x,grid_y,tmin_east,tmin_north,tmin_vals)
        message('- calculated TMIN grid via %s' % interp_method)
    elif interp_method == 'CSP':
        grid_prcp = process_grid_csp(grid_x,grid_y,prcp_east,prcp_north,prcp_vals)
        grid_prcp = np.where(grid_prcp < 0.0, 0.0, grid_prcp) # ensure prcp is a positive definite field
        message('- calculated PRCP grid via %s' % interp_method)
        grid_tmax = process_grid_csp(grid_x,grid_y,tmax_east,tmax_north,tmax_vals)
        message('- calculated TMAX grid via %s' % interp_method)
        grid_tmin = process_grid_csp(grid_x,grid_y,tmin_east,tmin_north,tmin_vals)
        message('- calculated TMIN grid via %s' % interp_method)
    elif interp_method == 'BSP':
        grid_prcp = process_grid_bsp(grid_x,grid_y,prcp_east,prcp_north,prcp_vals)
        grid_prcp = np.where(grid_prcp < 0.0, 0.0, grid_prcp) # ensure prcp is a positive definite field
        message('- calculated PRCP grid via %s' % interp_method)
        grid_tmax = process_grid_bsp(grid_x,grid_y,tmax_east,tmax_north,tmax_vals)
        message('- calculated TMAX grid via %s' % interp_method)
        grid_tmin = process_grid_bsp(grid_x,grid_y,tmin_east,tmin_north,tmin_vals)
        message('- calculated TMIN grid via %s' % interp_method)
    elif interp_method == 'IDW':
        grid_prcp = process_grid_idw(grid_x,grid_y,grid_shape,prcp_east,prcp_north,prcp_vals,3)
        grid_prcp = np.where(grid_prcp < 0.0, 0.0, grid_prcp) # ensure prcp is a positive definite field
        message('- calculated PRCP grid via %s' % interp_method)
        grid_tmax = process_grid_idw(grid_x,grid_y,grid_shape,tmax_east,tmax_north,tmax_vals,2)
        message('- calculated TMAX grid via %s' % interp_method)
        grid_tmin = process_grid_idw(grid_x,grid_y,grid_shape,tmin_east,tmin_north,tmin_vals,2)
        message('- calculated TMIN grid via %s' % interp_method)
    #
    ...
    message(' ')
#
message('process_NCEI_02_serial.py completed at %s' % datetime.datetime.now().isoformat())
message(' ')
sys.exit(0)
