...
USAGE: insert 'from Teleconnections import *' near head of script, then
       (for example)
        ice_on_doy, ice_off_doy, ice_duration = \
            get_ice_dates(fname, year_begin, (year_end + 1))
...
OUTPUT: data arrays returned to calling script
"""
...
def get_ao_idx(fname, yr_begin, yr_end):
    ...
    if yr_end <= yr_max:
        ao_winter_idx = ao_winter_idx[:(len(ao_winter_idx)
                                        - (yr_max - yr_end - 1))]
        ao_spring_idx = ao_spring_idx[:(len(ao_spring_idx)
                                        - (yr_max - yr_end - 1))]
        ao_summer_idx = ao_summer_idx[:(len(ao_summer_idx)
                                        - (yr_max - yr_end - 1))]
        ao_autumn_idx = ao_autumn_idx[:(len(ao_autumn_idx)
                                        - (yr_max - yr_end - 1))]
