...
PURPOSE: Plot T/P maps
              ...
              specially formatted series

DEPENDENCIES: sys, numpy, matplotlib, scipy
...
from UTM_Geo_Convert import geographic_to_utm, utm_to_geographic
mpl.use('Agg')
...
def p_map_plot(x, y, z, xmin, xmax, ymin, ymax, grid, UTMz,
               titlestr, fname, stations=1):
    plt.imshow(grid, extent=(xmin, xmax, ymin, ymax), origin='bottom',
    ...
    if stations:
        plt.hold(True)
        plt.scatter(x, y, s=20, c=z, cmap=plt.get_cmap('Blues'))
        plt.clim(0, 5)
        plt.hold(False)
...
def t_map_plot(x, y, z, xmin, xmax, ymin, ymax, grid, UTMz,
               titlestr, fname, stations=1):
    plt.imshow(grid, extent=(xmin, xmax, ymin, ymax), origin='bottom',
    ...
    if stations:
        plt.hold(True)
        plt.scatter(x, y, s=20, c=z, cmap=plt.get_cmap('jet'))
        plt.clim(-20, 40)
        plt.hold(False)
    message('saving figure as %s' % fname)
    plt.savefig(fname, dpi=300, bbox_inches='tight')
...
def masked_map_plot_utm(img, mask, UTM_zone, UTM_bounds, cmapname, cbar,
                        titlestr, fname, notitle=1):
...
def masked_map_plot_geo(img, mask, UTM_zone, UTM_bounds, cmapname, cbar,
                        titlestr, fname, gfilter=0, notitle=0):
    UTM_W = UTM_bounds[0]
    UTM_N = UTM_bounds[1]
    UTM_E = UTM_bounds[2]
    UTM_S = UTM_bounds[3]
    ...
    for lat in left_geog_ticks:
        lon = LL_lon + (((LL_lon - UL_lon) /
                         (LL_lat - UL_lat)) * (lat - LL_lat))
        _, east, north = geographic_to_utm(lon, lat, UTM_zone)
        left_utm_ticks.append(np.round(north, 0))
    # bot_geog_ticks
    bot_utm_ticks = []
    bot_geog_ticks = np.arange(np.ceil(LL_lon), np.ceil(LR_lon))
    for lon in bot_geog_ticks:
        lat = LL_lat + (((LR_lat - LL_lat) /
                         (LR_lon - LL_lon)) * (lon - LL_lon))
        _, east, north = geographic_to_utm(lon, lat, UTM_zone)
...
def p_ts_daily_plot(avg_pts, min_pts, max_pts, titlestr, fname, notitle=0):
    doy_min = 1
    doy_max = 365
    ...
    date_vequinox = 320
    doy_vequinox = date_to_doy(2013, date_vequinox)
    date_ssolstice = 621
    doy_ssolstice = date_to_doy(2013, date_ssolstice)
    date_aequinox = 922
    doy_aequinox = date_to_doy(2013, date_aequinox)
    date_wsolstice = 1221
    doy_wsolstice = date_to_doy(2013, date_wsolstice)
    ymin = 0.0
    ymax = 6.0
    plt.plot([doy_vequinox, doy_vequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_vequinox - 1, 4.0, 'vernal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_ssolstice, doy_ssolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_ssolstice - 1, 4.0, 'summer solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_aequinox, doy_aequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_aequinox - 1, 4.0, 'autumnal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_wsolstice, doy_wsolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_wsolstice - 1, 4.0, 'winter solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
...
def p_ts_annual_plot(winter, spring, summer, autumn, annual, annual_std,
                     year_min, year_max, titlestr, fname, notitle=0):
    years = np.arange(year_min, year_max + 1).astype(int)
    ymin = 0.0
    ymax = 120.0
    plt.errorbar(years, annual, yerr=annual_std, color='k', linestyle='-',
                 marker='o', markeredgecolor='k', markersize=5, linewidth=2,
                 elinewidth=1, label='Annual')
    plt.plot(years, winter, color='b', linestyle='-', marker='o',
             markeredgecolor='b', markersize=5, linewidth=2, label='Winter')
    plt.plot(years, spring, color='g', linestyle='-', marker='o',
             markeredgecolor='g', markersize=5, linewidth=2, label='Spring')
    plt.plot(years, summer, color='r', linestyle='-', marker='o',
             markeredgecolor='r', markersize=5, linewidth=2, label='Summer')
    plt.plot(years, autumn, color='y', linestyle='-', marker='o',
             markeredgecolor='y', markersize=5, linewidth=2, label='Autumn')
...
def t_ts_daily_plot(avg_pts, min_pts, max_pts, titlestr, fname, tbase=0.0,
                    fill=1, notitle=0):
    doy_min = 1
    doy_max = 365
    days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    ...
    if tbase > 0.0:
        ...
        if fill:
            x = np.linspace(doy_min - 1, doy_max, doy_max)
            plt.fill_between(x, tbase_pts, avg_pts, where=(avg_pts > tbase),
                             color='r', alpha=0.4)
            plt.fill_between(x, tbase_pts, avg_pts, where=(avg_pts < tbase),
                             color='b', alpha=0.3)
            plt.fill_between(x, zero_pts, avg_pts, where=(avg_pts < 0.0),
                             color='b', alpha=0.5)
...
def ttt_ts_daily_plot(avg_pts, min_pts, max_pts, m_plateau_beg, m_plateau_end,
                      m_last_frost, m_first_frost, titlestr, fname, tbase=0.0,
                      fill=1, notitle=0):
    doy_min = 1
    doy_max = 365
    days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
...
def shifted_cd_plot(avg_pts, min_pts, max_pts, m_plateau_beg, m_plateau_end,
                    m_plateau_cd, titlestr, fname, notitle=0):
    shift_month = 7
    shift_day = 1
    shift_mmdd = shift_month * 100 + shift_day
    date_reset = shift_mmdd
    doy_reset = date_to_doy(2013, date_reset)
    doy_min = 1
    doy_max = 365
    days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    doy_first_dom = [1]
    ...
    date_vequinox = 320
    doy_vequinox = date_to_doy(2013, date_vequinox) + offset
    date_ssolstice = 621
    doy_ssolstice = date_to_doy(2013, date_ssolstice) + offset
    date_aequinox = 922
    doy_aequinox = date_to_doy(2013, date_aequinox) + offset - doy_max
    date_wsolstice = 1221
    doy_wsolstice = date_to_doy(2013, date_wsolstice) + offset - doy_max
    ymin = 0.0
    ymax = 200.0
    plt.plot([doy_vequinox, doy_vequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_vequinox - 1, 60.0, 'vernal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_ssolstice, doy_ssolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_ssolstice - 1, 60.0, 'summer solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_aequinox, doy_aequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_aequinox - 1, 60.0, 'autumnal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_wsolstice, doy_wsolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_wsolstice - 1, 90.0, 'winter solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    avg_pts_shift = []
    ...
    plt.plot([plateau_begin, plateau_begin],
             [plateau_cd - 10, plateau_cd + 10], 'k-', linewidth=1)
    ...
    plt.text(plateau_end - 2, 20, 'mean plateau\nend date', fontsize=8,
             ha='right', va='top')
    plt.plot([offset - 90, offset - 20], [plateau_cd, plateau_cd], 'k-',
             linewidth=1)
    plt.text(offset - 55, plateau_cd + 1, 'mean plateau CD', fontsize=8,
             ha='center', va='bottom')
    plt.xlim([doy_min, doy_max])
...
def gdd_plot(avg_pts, min_pts, max_pts, m_plateau_beg, m_plateau_end,
             titlestr, fname, notitle=0):
    doy_min = 1
    doy_max = 365
    days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    doy_first_dom = [1]
    ...
    ymax = 2100.0
    plt.plot([doy_vequinox, doy_vequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_vequinox - 1, 600.0, 'vernal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_ssolstice, doy_ssolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_ssolstice - 1, 800.0, 'summer solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_aequinox, doy_aequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_aequinox - 1, 600.0, 'autumnal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_wsolstice, doy_wsolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_wsolstice - 1, 600.0, 'winter solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    ...
    plt.text(plateau_end + 2, 1700, 'mean plateau\nend date', fontsize=8,
             ha='left', va='top')
    plt.plot([doy_vequinox + 10, doy_ssolstice - 10],
             [np.max(avg_pts), np.max(avg_pts)], 'k-', linewidth=1)
    plt.text((doy_vequinox + doy_ssolstice) / 2.0, np.max(avg_pts) + 1,
             'mean warm season\nGDD accumulation', fontsize=8, ha='center',
             va='bottom')
    plt.xlim([doy_min, doy_max])
    plt.xticks(doy_first_dom)
    plt.xlabel('Day of Year')
...
def t_ts_bydecade_plot(d1_pts, d2_pts, d3_pts, titlestr, fname, notitle=0):
    doy_min = 1
    doy_max = 365
    days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    doy_first_dom = [1]
    ...
    plt.plot(zero_pts, 'k-', linewidth=1)
    plt.plot([doy_vequinox, doy_vequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_vequinox - 1, 15.0, 'vernal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_ssolstice, doy_ssolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_ssolstice - 1, -25.0, 'summer solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_aequinox, doy_aequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_aequinox - 1, -25.0, 'autumnal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_wsolstice, doy_wsolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_wsolstice - 1, 15.0, 'winter solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
...
def p_ts_bydecade_plot(d1_pts, d2_pts, d3_pts, titlestr, fname, notitle=0):
    doy_min = 1
    doy_max = 365
    days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    doy_first_dom = [1]
    ...
    ymax = 1.0
    plt.plot([doy_vequinox, doy_vequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_vequinox - 1, 0.7, 'vernal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_ssolstice, doy_ssolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_ssolstice - 1, 0.7, 'summer solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_aequinox, doy_aequinox], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_aequinox - 1, 0.7, 'autumnal equinox', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot([doy_wsolstice, doy_wsolstice], [ymin, ymax], 'k:', linewidth=1)
    plt.text(doy_wsolstice - 1, 0.7, 'winter solstice', fontsize=10,
             ha='right', va='bottom', rotation='vertical')
    plt.plot(d1_pts, 'b-', linewidth=2, label='1984-1993')
    plt.plot(d2_pts, 'g-', linewidth=2, label='1994-2003')
    plt.plot(d3_pts, 'r-', linewidth=2, label='2004-2013')
