"""
Python script 'process_NCEI_03_checkpointed.py'
...
USAGE: 'python process_NCEI_03_checkpointed.py NCEI_WLS_19830101-20151031 1983 ./grids'
...
message(' ')
message('process_NCEI_03_checkpointed.py started at %s' % datetime.datetime.now().isoformat())
message(' ')
#
ncpus = 8
tbase = 5.0
...
all_dates = numpy.copy(h5infile['dates'])
h5infile.close()
...
if len(vars_files) > 0:
    for vars_file in vars_files:
        if str(prev_year) in vars_file:
            ...
            varfname = vars_file
            break
if use_vars_file:  # if rolling accounting variable files exist to be carried over from previous year, use them
    message('extracting rolling accounting variable grids and datacubes from' % varfname)
    h5infile = hdf.File(varfname,'r')
    nrows = numpy.copy(h5infile['nrows'])
    ncols = numpy.copy(h5infile['ncols'])
    tmin_frz_prev = numpy.copy(h5infile['tmin_frz_prev'])
    tavg_frz_prev = numpy.copy(h5infile['tavg_frz_prev'])
    tmax_frz_prev = numpy.copy(h5infile['tmax_frz_prev'])
    chill_d_prev = numpy.copy(h5infile['chill_d_prev'])
    chill_dd_prev = numpy.copy(h5infile['chill_dd_prev'])
    grow_dd_prev = numpy.copy(h5infile['grow_dd_prev'])
    grow_dd_base0_prev = numpy.copy(h5infile['grow_dd_base0_prev'])
    tmin_90d = numpy.copy(h5infile['tmin_90d'])
    tavg_90d = numpy.copy(h5infile['tavg_90d'])
    tmax_90d = numpy.copy(h5infile['tmax_90d'])
    prcp_30d = numpy.copy(h5infile['prcp_30d'])
    prcp_90d = numpy.copy(h5infile['prcp_90d'])
    prcp_90d_nd0 = numpy.copy(h5infile['prcp_90d_nd0'])
    prcp_90d_nd10 = numpy.copy(h5infile['prcp_90d_nd10'])
    prcp_90d_nd25 = numpy.copy(h5infile['prcp_90d_nd25'])
    prcp_180d = numpy.copy(h5infile['prcp_180d'])
    prcp_365d_p1 = numpy.copy(h5infile['prcp_365d_p1'])
    prcp_365d_p2 = numpy.copy(h5infile['prcp_365d_p2'])
    h5infile.close()
...
message('saving rolling accounting variable grids and datacubes to' % varfname)
h5outfile = hdf.File(varfname,'w')
h5outfile.create_dataset('nrows', data=nrows)
h5outfile.create_dataset('ncols', data=ncols)
h5outfile.create_dataset('tmin_frz_prev', data=tmin_frz_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('tavg_frz_prev', data=tavg_frz_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('tmax_frz_prev', data=tmax_frz_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('chill_d_prev', data=chill_d_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('chill_dd_prev', data=chill_dd_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('grow_dd_prev', data=grow_dd_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('grow_dd_base0_prev', data=grow_dd_base0_prev, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('tmin_90d', data=tmin_90d, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('tavg_90d', data=tavg_90d, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('tmax_90d', data=tmax_90d, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_30d', data=prcp_30d, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_90d', data=prcp_90d, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_90d_nd0', data=prcp_90d_nd0, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_90d_nd10', data=prcp_90d_nd10, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_90d_nd25', data=prcp_90d_nd25, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_180d', data=prcp_180d, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_365d_p1', data=prcp_365d_p1, dtype=numpy.float32, compression='gzip')
h5outfile.create_dataset('prcp_365d_p2', data=prcp_365d_p2, dtype=numpy.float32, compression='gzip')
h5outfile.close()
...
#
message('process_NCEI_03_checkpointed.py completed at %s' % datetime.datetime.now().isoformat())
message(' ')
sys.exit(0)

# end process_NCEI_03_checkpointed.py
