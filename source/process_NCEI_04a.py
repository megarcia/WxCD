...
PURPOSE: Aggregating grids for specific dates and time periods over the analysis period,
         ...
         (in both days and GDD) of CD plateau. Numerous variables are addressed.
...
OUTPUT: New '.h5' file with aggregated grid datacubes
        (with the naming convention 'analyses/[YYYY]-[YYYY]_derived_clim_grids.h5')

RUN TIME: ~40 sec per data year
"""
...
message(' ')
message('process_NCEI_04a.py started at %s' % datetime.datetime.now().isoformat())
message(' ')
#
if len(sys.argv) < 4:
...
inputgridvars.append('prcp_90d_nd0_sum')
inputgridvars.append('prcp_90d_nd10_sum')
inputgridvars.append('prcp_90d_nd25_sum')
...
message(' ')
#
message('process_NCEI_04a.py completed at %s' % datetime.datetime.now().isoformat())
message(' ')
sys.exit(0)
