...
txt_files = ['ACKNOWLEDGEMENTS.txt','CITATION.txt','DISCLAIMER.txt','LICENSE_GnuGPLv3.txt']
md_files = ['README.md']
...
modules = ['Date_Convert.py','Interpolation.py','Plots.py','Read_Header_Files.py',
           'Stats.py','Teleconnections.py','UTM_Geo_Convert.py']
...
analyses_dirs = ['annual_maps','cluster_maps','ecoregion_maps','figures','summary_maps']
message(' ')
#
...
os.system('rm ._*')
os.system('rm */._*')
message('checking for essential auxiliary files that should accompany this software')
...
for md in md_files:
    if md in mds_present:
        ...
        absent += 1
if absent > 0:
    message('- you don\'t need them to run things, but you do need them to understand things')
    message('- you should probably download this package again from scratch')
    message('- exiting setup procedure')
    sys.exit(1)
message(' ')
#
message('checking for essential top-level directories that should already exist')
dirs_present = [ d.replace('/', '') for d in glob.glob('*/') ]
absent = 0
for dir in main_dirs:
    if dir in dirs_present:
        message('- found main directory \'%s\' as expected' % dir)
    else:
        message('- main directory \'%s\' is absent' % dir)
        absent += 1
if absent > 0:
...
for srcfile in tools:
    ...
    if srcfile in src_present:
        ...
        absent += 1
if absent > 0:
    message('- if you need these tools, you should download this package again from scratch')
message(' ')
#
...
try:
    ...
    message('- essential python dependency \'pp\' (ParallelPython) is not available')
    message('  (it\'s essential for now, until I develop serial versions of several routines)')
    err += 1
try:
    import gdal
...
try:
    ...
    message('- essential python dependency \'scipy.interpolate\' is not available')
    err += 1
try:
...
try:
    ...
    message('- essential python dependency \'matplotlib.pyplot\' is not available')
    err += 1
if err > 0:
    message('- you need to install one or more additional python packages for this software to work')
    message('- all of these packages are available via Anaconda (\'conda\') and/or PyPI (\'pip\') repositories')
    message('- exiting setup procedure')
    sys.exit(1)
message(' ')
#
...
message(' ')
#
message('checking for data documentation files that should accompany this software')
...
message(' ')
#
message('creating top-level and sub-directories that will be used for process output')
for dir in add_dirs:
    os.system('mkdir %s' % dir)
    message('- made top-level directory \'%s\' ' % dir)
for dir in analyses_dirs:
    os.system('mkdir analyses/%s' % dir)
    message('- made sub-directory \'analyses/%s\' ' % dir)
message(' ')
#
...
message('all set!')
message(' ')
message('make sure to read the \'README.md\' file before you get started on the scripts')
message(' ')
message('if you need help getting your own dataset of GHCND weather observations, there is')
message('    a how-to document in the \'docs\' directory')
message(' ')
message('please send questions, bug reports, any other requests to matt.e.garcia@gmail.com')
message('    (and include a helpfully descriptive subject line, if you could)')
message('or submit them through the Issues tab at the GitHub repository for this package')
message(' ')
sys.exit(0)
