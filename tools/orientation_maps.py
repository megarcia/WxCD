...
if len(sys.argv) < 3:
    message('input error: need h5 file containing study area boundary information')
    sys.exit(1)
else:
    maskfname = sys.argv[2]
#
if len(sys.argv) < 2:
    message('input error: need csv file containing processed station metadata information')
    sys.exit(1)
else:
    stnfname = sys.argv[1]
#
...
SWcorner = np.array([np.min([min_lat, sa_min_lat]), np.min([min_lon, sa_min_lon])], dtype=np.float32)
NEcorner = np.array([np.max([max_lat, sa_max_lat]), np.max([max_lon, sa_max_lon])+0.75], dtype=np.float32)
...
d1.drawcountries()
d1.drawstates()
d1.drawmapscale(lon=-95.2,lat=44.4,lon0=mid_lon,lat0=mid_lat,length=100.0,barstyle='fancy')
...
dPT = d1.scatter(lonsPT,latsPT,40,latlon=True,marker='o',c=nyPT,cmap=cmap,label=dPT_label)
dP_label = 'P only (%d)' % len(nyP)
dP = d1.scatter(lonsP,latsP,30,latlon=True,marker='s',c=nyP,cmap=cmap,label=dP_label)
dT_label = 'T only (%d)' % len(nyT)
dT = d1.scatter(lonsT,latsT,30,latlon=True,marker='^',c=nyT,cmap=cmap,label=dT_label)
cbar = d1.colorbar(dPT,location='bottom',pad="5%",cmap=cmap,norm=norm,boundaries=bounds,format='%1i')
