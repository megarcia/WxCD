...
PURPOSE: Query grids of climatological derivatives for given locations and dates
...
#     message('lat %.3f  lon %.3f  date %s' % (query_lats[i], query_lons[i], query_dates[i]))
# message(' ')
#
query_D = []
for i in range(0,len(query_dates)):
    ...
    mm = int(j[0])
    dd = int(j[1])
    datestr = '%d%s%s' % (yyyy, pad(mm), pad(dd))
...
for i in range(0,len(query_lats)):
    UTM_coords = geographic_to_utm(query_lons[i].astype(np.float64), query_lats[i].astype(np.float64), UTMzone)
...
for i in range(0,len(query_D)):
    datestr = query_D[i]
    message('lat %.3f  lon %.3f  date %s  -->  UTM zone %d  E %d  N %d  date %s' % (query_lats[i],query_lons[i],query_dates[i],UTMzone,query_E[i],query_N[i],datestr))
    wx_fname = 'grids/%s_NCEI_grids_2.h5' % datestr
    if wx_fname in wxlist:
        h5file = hdf.File(wx_fname,'r') 
        message('- extracting grid information from weather derivatives file %s' % wx_fname)
        ...
        if int(datestr[4:8] < 701):
            ...
            h5file.close()
            cd.append(chill_d_0630[query_row,query_col] + chill_d[query_row,query_col])    
            cdd.append(chill_dd_0630[query_row,query_col] + chill_dd[query_row,query_col])    
